...
def similaridades(modelo, vetor, norma_matriz):
    ...
    similaridades = np.zeros(modelo.shape[1])
    for documento in range(modelo.shape[1]):
        coluna = modelo[:, documento]
        norma_doc = norma_matriz[documento]
        if norma_doc != 0:
            similaridades[documento] = np.dot(vetor, coluna) / (norma_vetor * norma_doc)
...
with open(resultados_arquivo, 'w', newline='') as arquivo_res:
    ...
    for consulta in consultas_df.itertuples():
        ...
        doc_indices = sorted(range(len(similaridades_res)), key=lambda i: similaridades_res[i], reverse=True)[:100]
        for rank, doc_idx in enumerate(doc_indices, start=1):
            escritor.writerow([consulta.QueryNumber, rank, doc_idx, similaridades_res[doc_idx]])
